...
export default function BookDetails() {
  ...
  function handleSimilar() {
    ...
    if (book.volumeInfo.categories) {
      const searchString = book.volumeInfo.categories
        ...
        .filter(
          (word) =>
            !word.includes("General") &&
            !word.includes("Fiction") &&
            !word.includes("Women") &&
            !word.includes("Nonfiction") &&
            !word.includes("Juvenile"),
        );
  ...
  useEffect(() => {
    ...
    async function checkIfSaved() {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/api/saved/${id}`,
        );
        if (!response.ok) {
  ...
  async function handleDelete() {
    try {
      const result = await fetch(
        `${import.meta.env.VITE_API_URL}/api/saved/${book.id}`,
        {
          method: "DELETE",
        },
      );
          ...
          <div className="details-img-desc-btn">
            ...
            <div className="details-img-btn">
              ...
              <div className="details-buttons">
                ...
                <button className="details-btn" onClick={() => handleSimilar()}>
                  Find similar
                </button>
              </div>
            </div>
